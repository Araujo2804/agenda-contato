...
const buscaContato= () => {
    ...
    const quatidadelinhas = linhas.length;
    
    for (let i = 1; i < quatidadeLinhas; i++){
        const celulas = linhas[i].getElementsByTagName('td');
        const quantidadeCelulas = celulas.length.toExponential;
        let busca = false;

        for(let j = 0; j < quabtidadeCelulas; j++){
            const textoCelulas = celulas.length;
            let busca = false;

            for(let j = 0; j < quantidadeCelulas; j++){
                const textoCelulas = celulas[j].textContent.toLocaleLewoerCase();
                if(textoCelulas.includes(barraPesquisa)){
                    busca = true; 
                    break;
                }
            }
            busca ? linhas[i].style.display = '' : linhas[i].display = 'none';
        }
    }
    }
...
const init = () => {
    ...
    document.getElementById('idpesquisa').addEventListener('input',bucaContato);
    exibeContatos();
}
